...
ctxr.stroke();
 ...
 fleche_right = document.getElementById('fleche-right-prim');

ctxr = fleche_right.getContext('2d');
ctxr.beginPath();
ctxr.moveTo(5,0);
ctxr.lineTo(35,30);
ctxr.lineTo(35,25);
...
EssaiLineaire.prototype={
  constructor:EssaiLineaire,
    ajouter_algo_rech:function(){
        ...
        algo.innerHTML='<p id="l0">Adr = hachage(cle) ;</p>' +
            ...
            '<p id="l4">&nbsp &nbsp &nbsp &nbsp adr = adr -hachage\'(cle) ; </pid>' +
    ...
    ajouter_algo_insere:function(){
        ...
        algo.innerHTML='<p id="l0">SI ( nombre de case vide > 1 ) :</p>' +
            ...
            '<p id="l5">Sinon: Ecrire(\'le tableau est plein\');FIN;</p>' ;
    },
    ajouter_algo_supp:function(){
        let algo=document.getElementById('algo');
        algo.innerHTML='<p id="l0">Recherche( cle )</p>' +
            '<p id="l1">Si on trouve la cle : </p>' +
            '<p id="l2">&nbsp &nbsp Tab[adr].eff = 1;</p>' +

            '<p id="l3">Sinon:</p>'+
            '<p id="l4">&nbsp &nbsp Ecrire(\'la cle n\'existe pas\');</p>'+
            '<p id="l5">FIN;</p>'
        ;
    },
  ...
  hash: async function (cle,animtion=false) {
      ...
      await sleep(500);
        ...
        return cle % this.nbmax;
  },
  hashprim:async function(cle,animation=false){
    let a = await this.hash(cle);
    a*=a;
    a+=1;
    a=a % this.nbmax;
      if (animation) {
          document.getElementById('donne2-prim').innerText = a;

          document.getElementById('fleche-right-prim').style.opacity = 1;
          await sleep(500);
          document.getElementById('case-hach2-prim').style.opacity = 1;
          await sleep(500);
      }
    return a ;
  },
...
function affich_guide_struct() {
    ...
    guide.innerHTML='                    <div class="bg-modal">\n' +
        ...
        '                        <br>\n' +
        '                        - Double Hachage : Comme l\'essai linéaire mais la séquence cyclique des\n' +
        '                        <br>\n' +
        '                       cases à tester utilise un pas calculé par une 2e fonction de hachage h\'(x)\n' +
        '                        <br>\n' +
        '                       et la supperesseion est logique\n' +
        '                        <br>\n' +
        ...
        '                            <br>\n' +
        '                            - La case \'eff\' représente l\'effacement de la donnée. \n' +
        '                            <br>\n' +
        '                            - A droite du tableau, on laisse la trace des cases parcourus\n' +
        '                            <br>\n' +
        '                            <img class="mt-1" src='+imgurl()+' width="500">\n' +
        '                        </div>\n' +
        '\n' +
